...
public class Helper {
    ...
    public static boolean isTameable(World world, String entityTypeString) {
        if (!entityTypeString.isEmpty()) {
            ...
            if (resourceLocation != null) {
                return TAMEABLE.computeIfAbsent(resourceLocation, rl -> Registry.ENTITY_TYPE.getOptional(rl).map(entry -> {
    ...
    public static Optional<TextFormatting> fromColor(Color color) {
        return Stream.of(TextFormatting.values()).filter(f -> f.getColor() != null && f.getColor().equals(color.color)).findFirst();
    }
